...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const mockCaseDatabase = {
        ...
        'PPPS-2026-0118': {
            name: 'Zainab Bibi',
            type: 'Parole (Executive Stream)',
            officer: 'Sana Malik (APO, Faisalabad)',
            status: 'Vocational Placement & Training',
            statusClass: 'status-active',
            step: 3
        },
        'PPPS-2026-0789': {
            name: 'Kamran Khan',
            type: 'Probation (Judicial Stream)',
            officer: 'Tariq Mahmood (DPO, Rawalpindi)',
            status: 'Supervision Successfully Completed',
    ...
    trackerForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const rawQuery = caseIdInput.value.trim().toUpperCase();
        
        // Match registry keys
        const record = mockCaseDatabase[rawQuery];

        if (record) {
            document.getElementById('track-name').textContent = record.name;
            document.getElementById('track-type').textContent = record.type;
            document.getElementById('track-officer').textContent = record.officer;
            
            const statusEl = document.getElementById('track-status');
            statusEl.textContent = record.status;
            statusEl.className = `status-indicator ${record.statusClass}`;

            // Reset step elements classes
            for (let i = 1; i <= 4; i++) {
            ...
            alert('Case ID or CNIC not matched. Try demo cases:\n- PPPS-2026-0042\n- PPPS-2026-0118\n- PPPS-2026-0789');
        }
    });
